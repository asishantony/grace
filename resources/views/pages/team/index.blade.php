                ...
                <div class="responsive-table">
                    <table id="teams-table" class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{\Illuminate\Support\Str::limit($team->name,10)}}</td>
                                <td>{{\Illuminate\Support\Str::limit($team->description,20)}}</td>
                                {{-- <td><a href="{{asset('page-users-view')}}">dean3004</a>
                                </td> --}}
                                <td><img src="{{asset('storage/'.$team->image)}}" alt="{{$team->title}}" height="100px"></td>
                                <td style="cursor: pointer;"> @if ($team->status == "1")
                                    <span class="chip green lighten-5 status-change" data-id="{{$team->id}}">
                                        <span class="green-text">Active</span>
                                    </span>
                                    @else
                                    <span class="chip red lighten-5 status-change" data-id="{{$team->id}}">
                                        <span class="red-text">Inactive</span>
                                    </span>
                                    @endif
                                </td>
                                <td>
                                    <a href="/admin/teams/{{$team->id}}/edit"><span style="cursor: pointer;" data-id="{{$team->id}}" class=""><i
                                            class="material-icons">edit</i></span></a>
                                    <span style="cursor: pointer;" data-id="{{$team->id}}" class="view-team"><i
                                            class="material-icons">remove_red_eye</i></span>
                                    <span style="cursor: pointer;" data-id="{{$team->id}}" class="delete"
                                        data-id="{{$team->id}}"><i class="material-icons color-red">delete</i></span>
                                </td>
                            </tr>
                            @endforeach
                            @endif
...
<div class="modal-footer">
    <div class="card-action pl-0 pr-0 right-align">
        ...
        <button class="btn-small waves-effect waves-light save-teams-item" type="submit">
...
<script src="{{asset('js/scripts/teams.js')}}"></script>
@endsection
